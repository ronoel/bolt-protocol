...
@Component({
  selector: 'app-bolt-wallet-ui-fee-fund',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './bolt-wallet-ui-fee-fund.component.html',
  styleUrls: ['./bolt-wallet-ui-fee-fund.component.scss']
})
export class BoltWalletUiFeeFundComponent {
  @Input() stacksWalletBalance: bigint = BigInt(0);
  @Input() feeFundBalance: bigint = BigInt(0);
  ...
  public onSubmit() {
    ...
    this.boltContractSBTCService.depositFeeFund(atomicAmount).subscribe({
      next: (response) => {
        if (response.txid) {
          this.transactionSubmitted.emit({
            ...
            title: 'Deposit to Fee Fund'
          });
        }
        this.isProcessing = false;
      },
      error: (error) => {
        console.error('Error sending fee fund deposit transaction:', error);
