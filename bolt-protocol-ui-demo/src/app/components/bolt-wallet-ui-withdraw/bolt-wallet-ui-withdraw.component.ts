...
@Component({
  selector: 'app-bolt-wallet-ui-withdraw',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './bolt-wallet-ui-withdraw.component.html',
  styleUrls: ['./bolt-wallet-ui-withdraw.component.scss']
})
export class BoltWalletUiWithdrawComponent {
  ...
  public onSubmit() {
    ...
    // Perform bolt-to-stacks transfer
    this.boltContractSBTCService.transferBoltToStacks(
      atomicAmount,
      recipientAddress
    ).subscribe({
      next: (response) => {
        ...
        if (response.txid) {
          this.transactionSubmitted.emit({
            ...
            title: 'Withdraw to Stacks Wallet'
          });
        }
